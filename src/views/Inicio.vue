<template>
    <body>
    <header>
            <a href="https://www.uleam.edu.ec/"  class="logo">
            <img src="../assets/logouleam.png" alt="Logo Uleam" class="imglogo">
        </a>
        <h3 class="titulo">Gestion de inventario Mobiliario</h3>           
    </header>
    <div class="eleccion" align ="center">
        <p>Bienvenido al Sistema de inventario de Inmobiliario de la Uleam
                    por favor Inicie Sesion o registrese 
        </p>
        <router-link to="/sesion" class="boton">Iniciar Sesion</router-link>

        <router-link to="/registro" class="boton">Registrarse</router-link>
    </div>
    <footer>
        <p>© 2023 Miguel Angel Ortega Giler</p>
    </footer>
</body>
</template>
<script>
export default {
    name : "inicio"
}
</script>

<style scoped>
header{
    display: flex;
    height: 50px;
    background-color: rgb(237, 114, 48);
    justify-content: space-between;
    align-items: center;
    padding: 10px;
}
body{
    font-family: sans-serif;
    margin: 0;
    padding: 0;
    background-image: url(../assets/entrada.jpg);
    background-repeat: no-repeat;
    background-size: cover;
    display: flex;
    flex-direction: column;
    min-height: 100vh;
}
a{
    text-decoration: none;
}
.logo{
    display: flex;
    align-items: center;
}
.imglogo{
    height: 50px;
    margin-right: 10px;
}
.eleccion {
    width: 50%;
    margin: 180px auto;
    padding: 20px;
    border-radius: 10px;
    background: rgba(255, 255, 255, 0.8);
}
.boton {
    display: block;
    width: 25%;
    padding: 10px;
    margin-top: 10px;
    background-color: #333;
    color: white;
    border: none;
    border-radius: 5px;
    cursor: pointer;
}
.boton:hover {
    background-color: #555;
}
footer {
    background-color: #333;
    color: white;
    text-align: center;
    padding: 1px;
    margin-top: auto;
}
</style>